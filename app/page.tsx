...
function DashboardContent() {
	const { user, signOut, markOnboardingComplete, onboardingState } =
		useAuth();
	const {
		tasks,
		categories,
	...
	useEffect(() => {
		// Wait until the user exists and the onboarding status has been checked
		if (
			user &&
			!onboardingState.isCheckingOnboardingStatus &&
			!hasShownOnboarding
		) {
			// If the user has not completed onboarding, show the onboarding flow
			if (!onboardingState.hasCompletedOnboarding) {
				setShowOnboarding(true);
				setHasShownOnboarding(true); // Prevents re-showing in the same session
			}
		}
	}, [
		user,
